  ...
  return (
    <div className="doc-preview">
      ...
      <table><tbody>
        ...
        <tr><td style={{fontWeight:600}}>Fund Manager</td><td>{gpName}</td></tr>
      ...
      <table><tbody>
        ...
        <tr><td style={{fontWeight:600}}>Catch-Up</td><td>{(setup.catchUp ?? ideation.catchUp) ? '100% to Fund Manager until carried interest equals ' + formatPercent(setup.carryPerformanceFee || ideation.carryRate) + ' of total profits' : 'None'}</td></tr>
        <tr><td style={{fontWeight:600}}>Fund Manager Commitment</td><td>{formatPercent(setup.gpCommit || ideation.gpCommitment)} of total commitments</td></tr>
      ...
      <p><strong>Step 3 - Fund Manager Catch-Up:</strong> {(setup.catchUp ?? ideation.catchUp) ? `100% to the Fund Manager until the Fund Manager has received ${formatPercent(setup.carryPerformanceFee || ideation.carryRate)} of all amounts distributed in Steps 2 and 3.` : 'Not applicable.'}</p>
      <p><strong>Step 4 - Carried Interest:</strong> {formatPercent(100 - (setup.carryPerformanceFee || ideation.carryRate))}% to Limited Partners and {formatPercent(setup.carryPerformanceFee || ideation.carryRate)}% to the Fund Manager.</p>

      <h2>6. Governance</h2>
      <table><tbody>
        <tr><td style={{width:'40%', fontWeight:600}}>Advisory Board</td><td>{setup.advisoryBoardComposition || '3 investor representatives'}</td></tr>
        ...
        <tr><td style={{fontWeight:600}}>Fund Manager Removal</td><td>{setup.removalRights || '75% in interest of investors, with or without cause'}</td></tr>
      </tbody></table>
      ...
      <table><tbody>
        <tr><td style={{width:'40%', fontWeight:600}}>Investor Transfers</td><td>{setup.lpTransferRights || 'Subject to Fund Manager consent'}</td></tr>
...
export function generateServiceProviderBrief(fundData) {
  ...
  return (
    <div className="doc-preview">
      ...
      <h2>2. Key Parties</h2>
      <table><tbody>
        <tr><td style={{width:'40%', fontWeight:600}}>Fund Manager</td><td>{setup.gpName || '[Fund Manager Name]'}</td></tr>
...
export function generateSubscriptionAgreement(fundData) {
  ...
  return (
    <div className="doc-preview">
      ...
      <p><strong>TO:</strong> {gpName} (the "Fund Manager")</p>
      ...
      <p>The Investor hereby represents and warrants to the Fund Manager as follows:</p>
      ...
      <ul style={{paddingLeft: 20, marginBottom: 12}}>
        <li>Capital Contributions will be drawn down by the Fund Manager as needed during the Investment Period of {setup.investmentPeriod || ideation.investmentPeriod} years;</li>
        <li>The Fund Manager will provide not less than 10 business days' notice of each capital call;</li>
...
export function generateASICChecklist(fundData) {
  ...
  const wholesaleChecklist = [
    { category: 'Entity Setup', items: [
      'Register the Fund Manager entity (company) with ASIC',
  ...
  return (
    <div className="doc-preview">
      ...
      <ul style={{paddingLeft: 20, marginBottom: 12}}>
        ...
        <li>Provide regular reporting to the Fund Manager and Investor Advisory Committee</li>
      ...
      <h2>7. Termination</h2>
      <p>This Agreement may be terminated by: (a) mutual written agreement; (b) the Fund, upon removal of the Fund Manager in accordance with the Fund Agreement; (c) either party, upon a material breach that remains uncured for 30 days after written notice.</p>
...
export function generateLPA(fundData) {
  ...
  return (
    <div className="doc-preview">
      ...
      <p>The Partners hereby form a limited partnership under the laws of {setup.domicile || 'Australia'}, to be known as "{fundName}". The Fund Manager shall be {setup.gpName || '[Fund Manager Name]'}.</p>

      <h3>4. Capital Commitments</h3>
      <p>The target aggregate Capital Commitments of the Partnership shall be {formatCurrency(ideation.fundSize)}{setup.fundSizeMin ? `, with a minimum of ${formatCurrency(setup.fundSizeMin)}` : ''}{setup.fundSizeMax ? ` and a maximum of ${formatCurrency(setup.fundSizeMax)}` : ''}. The Fund Manager shall commit not less than {formatPercent(setup.gpCommit || ideation.gpCommitment)} of total commitments.</p>

      <h3>5. Management Fee</h3>
      <p>The Partnership shall pay to the Fund Manager (or its designee) a management fee of {formatPercent(ideation.managementFee)} per annum, calculated on {setup.mgmtFeeCalcBasis || 'Committed Capital during the Investment Period and on Invested Capital thereafter'}.</p>
...
export function generateSideLetter(fundData) {
  ...
  return (
    <div className="doc-preview">
      ...
      <p>The Fund Manager, {gpName}, hereby agrees to the following modifications to the terms of the LPA as they apply to the Investor:</p>

      <h2>1. Most Favoured Nation</h2>
      <p>The Fund Manager shall provide the Investor with copies of any side letters entered into with other Limited Partners that contain terms more favourable than those provided herein, and the Investor shall have the right to elect to receive any such more favourable terms.</p>

      <h2>2. Co-Investment Rights</h2>
      <p>[The Investor shall have the right to participate in co-investment opportunities alongside the Fund, subject to the Fund Manager's allocation policy. / Not applicable.]</p>
      ...
      <h2>4. Reporting</h2>
      <p>[Standard reporting applies. / In addition to the standard reporting, the Fund Manager shall provide the Investor with [additional reporting requirements].]</p>

      <h2>5. Transfer</h2>
      <p>[Standard transfer restrictions apply. / The Investor shall have the right to transfer its interest to affiliates without the prior consent of the Fund Manager.]</p>
